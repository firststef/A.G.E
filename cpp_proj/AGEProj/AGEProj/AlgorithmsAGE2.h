...
namespace AGE2
{
	...
	template<unsigned N, template<unsigned V> class Function>
	float adaptive_genetic_algorithm() {
		...
		for (unsigned run = 0; run < 15; run++) {
			...
			do
			{
				...
				for (unsigned i = 0; i < pop_size; i++)
				{
					...
					values[i] = pressure_function::call(arr);
					if (values[i] < min)
						min = values[i];
					else if (values[i] > max)
						max = values[i];
	...
	template<unsigned N, template<unsigned V> class Function>
	vector<float> traceable_adaptive_genetic_algorithm() {
	...
	struct AlgorythmAnalyzerAGE2p
	{
		nlohmann::json create_json()
		{
			...
			REGISTER_ALGORITHM(adaptive_genetic_algorithm, 10);
			REGISTER_ALGORITHM(adaptive_genetic_algorithm, 30);
			TRACE_ALGORITHM(adaptive_genetic_algorithm, 30, 100);
			printf("Ended GA on %s\n", Function<1>::name);
			...
			nlohmann::json j = {
				...
				{"global_minimum", std::to_string(Function<1>::global_minimum)},
				{"analysis",	nlohmann::json::array({
				DUMP_ANALYSIS_TO_JSON(adaptive_genetic_algorithm, 5),
				DUMP_ANALYSIS_TO_JSON(adaptive_genetic_algorithm, 10),
				DUMP_ANALYSIS_TO_JSON(adaptive_genetic_algorithm, 30),
				DUMP_TRACE_TO_JSON(adaptive_genetic_algorithm, 30, run_evolution_)
				})
				}
			};
