...
std::vector<float> CONCAT4(less_run_evolution_,algorithm,_,V);\
...
namespace AGE1
{
	...
	std::vector<float> traceable_hillclimb_best_improve(Problem<N>& problem)
	{
		...
		do
		{
			...
			run_evolution.push_back(best_minimum);
	...
	std::vector<float> traceable_simulated_annealing(Problem<N>& problem)
	{
		...
		do
		{
			...
			Temperature *= 0.99f;
		} while (Temperature > 0.365);//Terminating cond
